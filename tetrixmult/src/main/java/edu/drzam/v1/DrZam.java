...
public class DrZam extends JFrame {
	...
	/**
	 * Creates a new DrZam instance. Sets up the window's properties, and adds a
	 * controller listener.
	 */
	private DrZam() {
		/**
		...
		addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent e) {
				 ...
				 * paused and that there is no drop cooldown, then set the logic
				 * timer to run at a speed of 25 cycles per second.
				 */
				 ...
				 * position is valid. If so, we decrement the current column by
				 * 1.
				 */
				case KeyEvent.VK_LEFT:
					if (!isPaused
							&& liveBoard.isValidAndEmpty(currentType, currentCol - 1, currentRow, currentRotation)) {
						currentCol--;
					}
					break;
				 ...
				 * position is valid. If so, we increment the current column by
				 * 1.
				 */
				 ...
				 * playing a game. If so, toggle the pause variable and update
				 * the logic timer to reflect this change, otherwise the game
				 * will execute a huge number of updates and essentially cause
				 * an instant game over when we unpause if we stay paused for
				...
				/*
				 * Start Game - When pressed, check to see that we're in either
				 * a game over or new game state. If so, reset the game.
				 */
	...
	private void updateGame() {
		...
		if (liveBoard.isValidAndEmpty(currentType, currentCol, currentRow + 1, currentRotation)) {
			 ...
			 * 800pts]).
			 */
			int deletes = liveBoard.deleteConsecutivePills();
			if (deletes > 0) {
				score += 50 << deletes;
			}
